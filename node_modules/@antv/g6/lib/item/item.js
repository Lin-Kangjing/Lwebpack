...
function () {
  ...
  _proto.update = function update(cfg) {
    ...
    var newModel = Util.mix({}, model, cfg);
    ...
    if (onlyMove) {
      ...
      if (shapeFactory.shouldUpdate(shape) && newModel.shape === this.get('currentShape')) {
        var updateCfg = this.getShapeCfg(newModel); // 如果 x,y 发生改变，则重置位置
        ...
        shapeFactory.update(shape, updateCfg, this); // 设置 model 在更新后，防止在更新时取原始 model

        this.set('model', newModel); // 更新后重置节点状态

        this._resetStates(shapeFactory, shape);
      } else {
        // 如果不满足上面 3 种状态，重新绘制
        this.set('model', newModel); // 绘制元素时，需要最新的 model
  ...
  _proto.afterUpdate = function afterUpdate() {} // 是否仅仅移动
  ;
