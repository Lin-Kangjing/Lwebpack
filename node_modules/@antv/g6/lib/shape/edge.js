...
var vec2 = Util.vec2;
var DIFF_POINT_PERCENT = 0.01; // 计算切线使用
...
var singleEdgeDefinition = Util.mix({}, SingleShapeMixin, {
  ...
  getLabelStyleByPosition: function getLabelStyleByPosition(cfg, labelCfg, group) {
    ...
    var point = pathShape.getPoint(pointPercent);
    ...
    if (refX || refY) {
      // 进行偏移时，求偏移向量
      var offset = this._getOffset(refX, refY, tangent);
    ...
    var angle = vec2.angleTo([1, 0], tangent);

    var textAlign = this._getTextAlign(labelPosition, angle, autoRotate);
  ...
  // 根据相对偏移量
  _getOffset: function _getOffset(refX, refY, tangent) {
    refX = refX || 0; // 用户没有两个偏移量都指定的时候，保证正确

    refY = refY || 0;
    var perpendicular = [-tangent[1], tangent[0]]; // (x,y) 顺时针方向的垂直线 (-y, x);

    var out = []; // gl-matrix 的接口定义如果返回结果是 vector ， xxx(out, a, b); 所以必须事先定义返回量
  ...
  // 根据角度和对齐方式自动获取旋转角度
  _getAutoRotate: function _getAutoRotate(labelPosition, textAlign, angle) {
    var rotate = 0;
